...
def nvinfer_element_configurator(
    ...
    if label_file and not label_file_path.is_file():
        raise NvInferConfigException(f'Label file "{label_file_path}" not found.')
    ...
    if issubclass(model_type, AttributeModel):
        if label_file:
            ...
            if len(attr_labels) != len(model_config.output.attributes):
                raise NvInferConfigException(
                    'Number of labels in label file does not match '
                    'the number of model output attributes.'
                )
            for attr, labels in zip(model_config.output.attributes, attr_labels):
                if attr.labels:
                    logger.warning(
                        'Model output attribute labels for "%s" '
    ...
    elif issubclass(model_type, ObjectModel):
        ...
        if model_config.output.objects:
            ...
            # load model object labels from file
